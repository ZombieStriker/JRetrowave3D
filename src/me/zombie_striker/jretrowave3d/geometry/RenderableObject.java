...
import me.zombie_striker.jretrowave3d.rotation.KernelRotatePitchYaw;
import me.zombie_striker.jretrowave3d.rotation.KernelRotateYawPitch;
...
public abstract class RenderableObject{
	...
	public void updateTriangles(boolean useGPU) {
			...
			for (int i = 0; i < t.getTrues().length; i++) {
				...
				if(useGPU){
					if(isRelativeToScreen){
						rotatableWrapperScreenRel.add(t.getRelativeLocation()[i],getCenter(JRetroWave3D.getGame().getWorld()),getYaw(),getPitch());
					}else{
						rotatableWrapper1.add(t.getRelativeLocation()[i],getCenter(JRetroWave3D.getGame().getWorld()),getYaw(),getPitch());
					}
				}else {
